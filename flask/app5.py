...
#                 # Check if the model is an autoencoder or denoising autoencoder
...
#                     # Render the result template with the original and autoencoded images
#                     return render_template('result.html', original_image=original_img_path, autoencoded_image=decoded_img_path)
#                 elif 'denoising_autoencoder' in selected_model_name:
#                     # Convert the output back to an image
#                     denoised_img = prediction.reshape((prediction.shape[1], prediction.shape[2], prediction.shape[3]))
#                     denoised_img = (denoised_img * 255).astype(np.uint8)
#                     denoised_img = Image.fromarray(denoised_img)
#                     denoised_img_path = 'static/denoised_image.jpg'
#                     denoised_img.save(denoised_img_path)
#                     # Render the result template with the original and denoised images
#                     return render_template('result.html', original_image=original_img_path, denoised_image=denoised_img_path)
#                 else:
#                     # For other models, return the prediction result
#                     # Modify this part to handle other models as needed
#                     return f"Prediction for {uploaded_file.filename} using model {selected_model_name}: {prediction}"
#             except Exception as e:
